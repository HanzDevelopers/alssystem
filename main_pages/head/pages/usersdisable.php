...
// Query to get the total number of users with user_type = 'implementer' in the same district as the logged-in user
$search_query = "SELECT COUNT(*) AS total 
                 FROM user_tbl 
                 WHERE LOWER(user_type) = 'implementer' 
...
$sql = "SELECT user_id, user_name, email, phone_number, user_type, disable_date, enable_date, status, district 
        FROM user_tbl 
        WHERE LOWER(user_type) = 'implementer' 
        AND district = '" . $conn->real_escape_string($logged_in_district) . "'"; // Filter by district
    ...
    <table class="table table-striped table-bordered">
        ...
        <tbody>
            ...
            if ($result->num_rows > 0) {
                ...
                <td>" . htmlspecialchars($row["disable_date"]) . "</td>
                <td>" . htmlspecialchars($row["enable_date"]) . "</td>
                <td>
...
<script>
    ...
    // Send AJAX request to update the status and dates
    fetch('update_status.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
        },
        body: `user_id=${userId}&status=${newStatus}&date=${currentDate}`
    })
